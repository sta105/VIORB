...
public:
    ...
    int static PoseOptimization15DoF(Frame *pFrame, KeyFrame* pLastKF, const IMUPreintegrator& imupreint, const cv::Mat& gw, const bool& bComputeMarg=false);
    int static PoseOptimization15DoF(Frame *pFrame, Frame* pLastFrame, const IMUPreintegrator& imupreint, const cv::Mat& gw, const bool& bComputeMarg=false);
#endif

    void static GlobalBundleAdjustmentNavStateWithGw(Map* mMap, cv::Mat& gw, int nIterations=5, bool* pbStopFlag=NULL);
    ...
    void static LocalBundleAdjustmentNavState15DoF(KeyFrame *pKF, const std::list<KeyFrame*> &lLocalKeyFrames, bool* pbStopFlag, Map* pMap, cv::Mat& gw, LocalMapping* pLM=NULL);
