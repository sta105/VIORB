...
bool LocalMapping::TryInitVIO(void)
{
    ...
    //TODO: should copy data, in case changed by KF culling
    // or do not cull KF during initialization

//    // Use all KeyFrames in map to compute
//    vector<KeyFrame*> vScaleGravityKF = mpMap->GetAllKeyFrames();
//    int N = vScaleGravityKF.size();
//    // Do not use mpCurrentKeyFrame in parallel thread as it may change
//    KeyFrame* pNewestKF = vScaleGravityKF[N-1];
//    if(pNewestKF!=vScaleGravityKF.back()) std::cerr<<"pNewestKF!=vScaleGravityKF.back()"<<std::endl;
//    if(pNewestKF->mnId != mpCurrentKeyFrame->mnId) std::cerr<<"pNewestKF->mnId != mpCurrentKeyFrame->mnId. Need modification. "<<pNewestKF->mnId <<" vs "<< mpCurrentKeyFrame->mnId<<std::endl;
...
//             <<(t3-t0)/cv::getTickFrequency()*1000<<" "<<endl;
        ...
        ofstream fRwi("/home/jp/opensourcecode/ORB_SLAM2/tmp/Rwi.txt");
...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpMap->KeyFramesInMap()>2)
                {
                    ...
                    else
                    {
                        ...
                        //Optimizer::LocalBundleAdjustmentNavState15DoF(mpCurrentKeyFrame,mlLocalKeyFrames,&mbAbortBA, mpMap, mGravityVec, this);
                ...
                if(!GetVINSInited())
                {
                    ...
                    if(tmpbool)
                    {
                        ...
                        SetFirstVINSInited(true);
                        cout<<GetMapUpdateFlagForTracking()<<", "<<GetFirstVINSInited()<<endl;
                    }
                }
#endif
            ...
            //std::cout << "LocalMapping Time difference = " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() <<std::endl;
        }
        else if(Stop())
        {
            cerr<<"wait stop here"<<endl;
            // Safe area to stop
...
//            //TODO
//            //min/max distance of mappoint
//            if(GetVINSInited() && (z1<0.5 || z2<0.5 /*|| (z1>10&&z2>10)*/))
//                continue;
...
{
//    // Need consideration. Below will cause multi-tread deadlock.
//    // xxDon't cull KeyFrames while running GBA.
...
//        {
//            //double timegap=0.02;
//            double timegap=0.5; //tested good, 0.5 & 0.1 below
//            if(GetVINSInited() && pKF->mTimeStamp < mpCurrentKeyFrame->mTimeStamp - 10.0)
//                timegap = 3;
//            //double timegap=1; //first tested, not so good
//            //double timegap=3; //second tested, bad
...
//            {
//                //timegap = 0.02; // with this value, kf culling is disabled
//                timegap = 0.1;    // third tested, good
//                //timegap = 0.2;  // first tested, not so good
//                //timegap = 0.5;    //second tested, bad
//                if(GetVINSInited())
//                    timegap = 0.5;
//                // Test log
...
//            if(fabs(pNextKF->mTimeStamp - pPrevKF->mTimeStamp) > timegap  && pKF->mTimeStamp < mpCurrentKeyFrame->mTimeStamp - 10.0)
//                continue;
//        }
...
//                            // do not account for keyframes in sliding window
//                            if(pKFi->mnId >= pOldestLocalKF->mnId)
//                                continue;
