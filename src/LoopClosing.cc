...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            //std::cout << "LoopClosing Time difference = " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() <<std::endl;
        }
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
            ...
            while(!lpKFtoCheck.empty())
            {
                ...
                else cout<<"pKF->mnBAGlobalForKF != nLoopKF???"<<endl;
