...
{
    //unique_lock<mutex> lock(mpMap->mMutexMapUpdate); // no need. called after Track finished in main()
    slamdata.TrackingStatus = mpTracker->mState;
    if(slamdata.TrackingStatus > 0)
    {
        if(!mpTracker->mCurrentFrame.mTcw.empty())
        {
            slamdata.Tcw = mpTracker->mCurrentFrame.mTcw.clone();
        }
        slamdata.VINSInitFlag = mpLocalMapper->GetVINSInited();
        if(slamdata.VINSInitFlag)
        {
            slamdata.gw = mpLocalMapper->GetGravityVec();
            slamdata.Rwi = mpLocalMapper->GetRwiInit();
        }
        slamdata.Timestamp = mpTracker->mCurrentFrame.mTimeStamp;
    }
}
...
{
    //std::chrono::steady_clock::time_point time1,time2,time3,time4,time5,time6;
...
//    }
    ...
    //std::cout << "time 1=" << std::chrono::duration_cast<std::chrono::microseconds>(time2 - time1).count() <<" before GrabImageMonoVI() in TrackMonoVI() "<<std::endl;
