...
bool Tracking::TrackLocalMapWithIMU(bool bMapUpdated)
{
    ...
    {
//        // Get initial pose from MapUpdateKF
//        KeyFrame* pMapUpdateKF = mpLocalMapper->GetMapUpdateKF();// Updated KF by Local Mapping. Should be the same as mpLastKeyFrame
//        // IMU since MapUpdateKF
//        vector<IMUData> vIMUSinceMapUpdateKF;
//        KeyFrame* pKF = pMapUpdateKF;
//        while(pKF->mnId != mpLastKeyFrame->mnId)
//        {
//            KeyFrame* pKFnext = pKF->GetNextKeyFrame();
//            vector<IMUData> vIMUthis = pKFnext->GetVectorIMUData();
//            vIMUSinceMapUpdateKF.insert(vIMUSinceMapUpdateKF.end(),vIMUthis.begin(),vIMUthis.end());
//            pKF = pKFnext;
//        }
//        vIMUSinceMapUpdateKF.insert(vIMUSinceMapUpdateKF.end(),mvIMUSinceLastKF.begin(),mvIMUSinceLastKF.end());
//        IMUPreintegrator imupreint = GetIMUPreIntSinceLastKF(&mCurrentFrame, pMapUpdateKF, vIMUSinceMapUpdateKF);
...
//            cerr<<"1 need to modify this part. LastKF in Tracking maybe not UpdateKF in Localmapping? lastKF id:"<<mpLastKeyFrame->mnId<<", MapUpdateKF id:"<<pMapUpdateKF->mnId<<endl;
        ...
        //Optimizer::PoseOptimization15DoF(&mCurrentFrame,pMapUpdateKF,imupreint,mpLocalMapper->GetGravityVec(),true);
...
//            }
//            vIMUSinceMapUpdateKF.insert(vIMUSinceMapUpdateKF.end(),mvIMUSinceLastKF.begin(),mvIMUSinceLastKF.end());
//            mIMUPreIntInTrack = GetIMUPreIntSinceLastKF(&mCurrentFrame, pMapUpdateKF, vIMUSinceMapUpdateKF);
//        }
...
bool Tracking::TrackWithIMU(bool bMapUpdated)
{
    ...
    if(mpLocalMapper->GetFirstVINSInited() || bMapUpdated)
    {
        ...
        //Optimizer::PoseOptimization15DoF(&mCurrentFrame,mpLastKeyFrame,mIMUPreIntInTrack,mpLocalMapper->GetGravityVec(),false);
...
IMUPreintegrator Tracking::GetIMUPreIntSinceLastKF(Frame* pCurF, KeyFrame* pLastKF, const std::vector<IMUData>& vIMUSInceLastKF)
{
    ...
    // remember to consider the gap between the last KF and the first IMU
    {
        ...
        if(dt < 0)
        {
            cerr<<std::fixed<<std::setprecision(3)<<"dt = "<<dt<<", last KF vs last imu time: "<<pLastKF->mTimeStamp<<" vs "<<imu._t<<endl;
    ...
    mIsFirstDebug(true)
{
    ...
    if(sensor==System::MONOCULAR)
        mpIniORBextractor = new ORBextractor(/*2*nFeatures*/2000,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    const bool c1a = mCurrentFrame.mTimeStamp>=mpLastKeyFrame->mTimeStamp+1.0;
...
void Tracking::saveDebugStates()
{
...

    {
        ...
        cv::Mat predictedRelativePose = mVelocity * 1;
        Eigen::Matrix3d predictedR;
        predictedR <<   predictedRelativePose.at<float>(0, 0), predictedRelativePose.at<float>(0, 1), predictedRelativePose.at<float>(0, 2), 
                        predictedRelativePose.at<float>(1, 0), predictedRelativePose.at<float>(1, 1), predictedRelativePose.at<float>(1, 2), 
                        predictedRelativePose.at<float>(2, 0), predictedRelativePose.at<float>(2, 1), predictedRelativePose.at<float>(2, 2);
